...
class CsvInvoiceCalculatorTest {
    ...
    @Test
    void calculatesItemAndGrandTotals() throws Exception {
        ...
        assertEquals(20.5, cell(result, 1, 5), 0.001);
        assertEquals(22.55, cell(result, 1, 6), 0.001);
    ...
    @Test
    void parsesVietnameseSemicolonCsvAndUsesVietnameseLabels() throws Exception {
        ...
        assertEquals(2.5, cell(result, 1, 4), 0.001);
        assertEquals(2.75, cell(result, 1, 5), 0.001);
        try (var wb = WorkbookFactory.create(new ByteArrayInputStream(result))) {
            assertEquals("Tổng trước thuế", wb.getSheetAt(0).getRow(0).getCell(4).getStringCellValue());
        }
    }
    ...
    @Test
    void acceptsZeroPriceAndMaximumVat() throws Exception {
        byte[] result = CsvInvoiceCalculator.process(data("Free", "1", "0", "100"), MAP, OutputLabels.english());
        assertEquals(0d, cell(result, 1, 4), 0.001);
        assertEquals(0d, cell(result, 1, 5), 0.001);
    }

    @Test
    void preservesAllSourceColumnsAndAddsCalculatedHeaders() throws Exception {
        ...
        try (var wb = WorkbookFactory.create(new ByteArrayInputStream(result))) {
            ...
            assertEquals("custom", sheet.getRow(1).getCell(5).getStringCellValue());
            assertEquals("Tổng trước thuế", sheet.getRow(0).getCell(6).getStringCellValue());
        }
    }

    @Test
    void roundsMoneyHalfUp() throws Exception {
        assertEquals(1.01, cell(CsvInvoiceCalculator.process(data("x", "1", "1.005", "0"), MAP, OutputLabels.english()), 1, 4), 0.001);
    }

    @Test
    void rejectsNonPositiveQuantity() {
        assertInvalid(data("x", "0", "1", "0"), "quantity");
        assertInvalid(data("x", "-1", "1", "0"), "quantity");
    }

    @Test
    void rejectsNegativePrice() {
        assertInvalid(data("x", "1", "-1", "0"), "unit price");
    }

    @Test
    void rejectsVatOutsidePercentageRange() {
        assertInvalid(data("x", "1", "1", "101"), "VAT");
        assertInvalid(data("x", "1", "1", "-1"), "VAT");
    }

    @Test
    void rejectsNonNumericRequiredValues() {
        assertInvalid(data("x", "one", "1", "0"), "quantity");
        assertInvalid(data("x", "1", "1x", "0"), "unit price");
        assertInvalid(data("x", "1", "1", "ten"), "VAT");
    }

    @Test
    void rejectsBlankNameAndMissingNumbers() {
        assertInvalid(data(" ", "1", "1", "0"), "product name");
        assertInvalid(data("x", "", "1", "0"), "quantity");
    }

    @Test
    void rejectsBadMappingAndUnevenRows() {
    ...
    @Test
    void rejectsEmptyOrUndetectableCsv() {
    ...
    @Test
    void rejectsInvalidVatPercentSyntax() {
        assertInvalid(data("x", "1", "1", "10%%"), "VAT");
    }
